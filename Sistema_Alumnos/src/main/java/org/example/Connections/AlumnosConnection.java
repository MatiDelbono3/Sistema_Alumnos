...
public class AlumnosConnection {
    ...
    public int insertarAlumnos(Alumnos alumno){
        ...
        try (Connection conexion= cn.Connect();
            ...
            java.sql.Date sqlFechaNacimiento = new java.sql.Date(alumno.getFecha_Nacimiento().getTime());
             ps.setDate(3, sqlFechaNacimiento);
             ps.setString(4, alumno.getCorreo_electronico());
            java.sql.Date sqlFechaInscripcion = new java.sql.Date(alumno.getFecha_Inscripcion().getTime());
             ps.setDate(5, sqlFechaInscripcion);
